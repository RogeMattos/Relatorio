...
export const getEstimatedExchangeRate = async (fromCurrency: string, toCurrency: string, date: string): Promise<number> => {
  try {
    ...
    if (!apiKey) {
        // Very rough static estimates to prevent app breakage without API Key
        ...
        // Return 1 if completely unknown and no API
        return 1.0;
    }
    ...
    // If quota exceeded, we silently fail to fallback 1.0 to not block the UI flow for rate, 
